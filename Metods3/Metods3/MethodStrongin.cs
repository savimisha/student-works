...
namespace Metods3
{
    class MethodStrongin : IMethod
    {
        ...
        public MethodStrongin(Function func, List<double> area, List<int> countIter, List<bool> isEpsilonCondition, List<double> r,
        ...
        public double CharactR(int i)
        {
            ...
            {
                return (_m * (X[i] - X[i - 1]) +
                        (1 / (double)_m) * ((_func.GetValue(_pointTest, X[i]) - _func.GetValue(_pointTest, X[i - 1])) *
                        (_func.GetValue(_pointTest, X[i]) - _func.GetValue(_pointTest, X[i - 1]))) / (X[i] - X[i - 1])
                        - 2 * (_func.GetValue(_pointTest, X[i]) + _func.GetValue(_pointTest, X[i - 1])));
            }
            else
            {
                return (_m * (X[i] - X[i - 1]) +
                        (1 / (double)_m) * ((_func.GetValue( X[i], _minPoint[i]) - _func.GetValue(X[i - 1], _minPoint[i - 1])) *
                        (_func.GetValue(X[i], _minPoint[i]) - _func.GetValue( X[i - 1], _minPoint[i - 1]))) / (X[i] - X[i - 1])
                        - 2 * (_func.GetValue( X[i], _minPoint[i]) + _func.GetValue( X[i - 1], _minPoint[i - 1])));
            }
        }
